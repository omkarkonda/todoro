  ...
  .controller('timerCtrl',[
    ...
    '$element',
     function($scope, $rootScope, dataservice, $interval, $element){
      ...
      $scope.alarm = new Audio('./sounds/Alarm_Clock.mp3');
      ...
      $scope.taskComplete = function(id){
        dataservice.todos = dataservice.todos.map(function(task){
          if(task.id == id && task.todoCompleted === false){
            task.todoCompleted = true;
            task.timeTaken = $scope.totalTimerCounter * $scope.tdMins;
            $scope.removeTask();
            $scope.showCompleteBtn = false;
            $rootScope.$emit('taskCompleted');
            $scope.totalTimerCounter = 0;
            return task;
          }else {
             return task;
